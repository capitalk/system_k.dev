...
std::string 
date_to_string(const dt::date& d)
{
...
bool 
is_bad_filename_char(char c) { 
    return c == '/' || c == '\\' || c == ' ';
} 

std::string  
remove_bad_filename_chars(const std::string& str) { 
...
void 
Application::onLogon(const FIX::SessionID& sessionID )
{
#ifdef DEBUG
...
void 
Application::onLogout(const FIX::SessionID& sessionID )
{
#ifdef DEBUG
...
void 
Application::onCreate(const FIX::SessionID& sessionID )
{
#ifdef DEBUG
...
void 
Application::fromAdmin(const FIX::Message& message, 
            ...
            FIX::RejectLogon )
{
	...
	if (beginString == FIX::BeginString_FIX42) {
		...
		((FIX50::MessageCracker&)(*this)).crack((const FIX44::Message&) message, sessionID);
	}
}

void 
Application::fromApp(const FIX::Message& message, 
        const FIX::SessionID& sessionID)
        throw(FIX::FieldNotFound, 
            FIX::IncorrectDataFormat, 
            FIX::IncorrectTagValue, 
            FIX::UnsupportedMessageType)
{
    ...
    if (_appMsgCount % 10000 == 0) {
        std::cout << "Received " << _appMsgCount << " application messages" << std::endl;;
    }
	FIX::BeginString beginString;
	message.getHeader().getField(beginString);
	if (beginString == FIX::BeginString_FIX42) {
		...
		((FIX50::MessageCracker&)(*this)).crack((const FIX50SP2::Message&) message, sessionID);
	}
}

void 
Application::toApp(FIX::Message& message, const FIX::SessionID& sessionID)
    throw(FIX::DoNotSend)
{
#ifdef DEBUG
    if (_config.print_debug) {
...
template <typename T> 
void 
Application::trading_session_status_template(const T& message, 
        const FIX::SessionID& sessionID)  
{
 ...
 */
void 
Application::onMessage(const FIX44::MarketDataSnapshotFullRefresh& message, 
        const FIX::SessionID& sessionID) 
{
#ifdef DEBUG
    if (_config.print_debug) {
        pan::log_DEBUG( "Application::onMessage(const FIX44::MarketDataSnapshotFullRefresh& message, ...)");
    }
#endif
 ...
 */
void 
Application::onMessage(const FIX43::MarketDataSnapshotFullRefresh& message, 
        const FIX::SessionID& sessionID) 
{
#ifdef DEBUG
    if (_config.print_debug) {
        pan::log_DEBUG( "Application::onMessage(const FIX43::MarketDataSnapshotFullRefresh& message, ...");
    }
#endif
...
void 
Application::onMessage(const FIX42::MarketDataSnapshotFullRefresh& message, 
        const FIX::SessionID& sessionID) 
{
#ifdef DEBUG
	if (_config.print_debug) { 
        pan::log_DEBUG("Application::onMessage(const FIX42::MarketDataSnapshotFullRefresh& message, ...",  "\n(", message.toString().c_str(), ")");
    }
#endif
...
template <typename T> 
void 
Application::full_refresh_template(const T& message, 
        const FIX::SessionID& sessionID) 
{
#ifdef DEBUG
    if (_config.print_debug) {
	...
	if (message.isSetField(noMDEntries)) {
        ...
        if (bbid > bask) {
            std::cout << "Crossed book: " 
                << pBook->getName() 
                << " (" 
                << (double) bbsize << "-" << (double) bbid 
                <<  "@" 
                << (double) basize << "-" << (double) bask 
                << ")" 
                << std::endl;
        }
...
void 
Application::broadcast_bbo_book(void* bcast_socket, 
        const char* symbol, 
 ...
 */
void 
Application::onMessage(const FIX44::MarketDataIncrementalRefresh& message, 
        const FIX::SessionID& sessionID) 
{
...
void 
Application::onMessage(const FIX43::MarketDataIncrementalRefresh& message, 
        const FIX::SessionID& sessionID) 
{
...
void 
Application::onMessage(const FIX42::MarketDataIncrementalRefresh& message, 
        const FIX::SessionID& sessionID) 
{
...
void 
Application::onMessage(const FIX44::MarketDataRequestReject& message, 
        const FIX::SessionID& sessionID) 
{
#ifdef DEBUG
	if (_config.print_debug) { 
...
void 
Application::onMessage(const FIX43::MarketDataRequestReject& message, 
        const FIX::SessionID& sessionID) 
{
#ifdef DEBUG
	if (_config.print_debug) { 
...
void 
Application::onMessage(const FIX42::MarketDataRequestReject& message, 
        const FIX::SessionID& sessionID) 
{
#ifdef DEBUG
	if (_config.print_debug) { 
