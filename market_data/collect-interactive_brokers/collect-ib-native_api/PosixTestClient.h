...
#include "proto/interactive_brokers.pb.h"
...
public:
    ...
    void subscribe(const TickerId id, const Contract& contract, const IBString& ticklist, const bool snapshot);
    int handleTickSize(const TickerId tickerId, TickType field, double size);
    int handleTickPrice(const TickerId tickerId, TickType field, double price, int canAutoExecute); 
	void reqCurrentTime();
private:
    void zmqPublishTick(const capk::IBSingleMarketBBO_t& ib);
//	void placeOrder();
	//void cancelOrder();
...
private:
    ...
    std::map<TickerId, capk::IBSingleMarketBBO_t> contractPriceMap;
	OrderId m_orderId;
    zmq::context_t m_zmqContext;
    zmq::socket_t m_pubSock;
};
