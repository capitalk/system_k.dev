...
{
	pan::log_DEBUG("KMsgProcessor::~KMsgProcessor()");
	if (_frontend) {
		_frontend->close();
		delete _frontend;
	}
...
void
KMsgProcessor::snd_HEARTBEAT_ACK(const strategy_id_t& sid)
{
	assert(_admin);
	bool rc;
...
void
KMsgProcessor::snd_STRATEGY_HELO_ACK(const strategy_id_t& sid)
{
	assert(_admin);
	bool rc;

#ifdef LOG
	pan::log_DEBUG("KMsgProcessor::snd_STRATEGY_HELO_ACK() to SID: ", 
...
KMsgProcessor::req()
{
...
#ifdef LOG
	pan::log_DEBUG("KMsgProcessor::req() frontend rcvd route header[", 
	...
	capk::msg_t msgType = *(static_cast<capk::msg_t*>(msg_type.data()));	
		...
		pan::log_DEBUG("KMsgProcessor::req() rcvd msg_type: ", pan::integer(msgType));
		pan::log_DEBUG("KMsgProcessor::req() rcvd SID [", pan::integer(sidframe.size()), "]: ", 
...
KMsgProcessor::rcv_internal()
{
    ...
    assert(rc);
#ifdef LOG
	pan::log_DEBUG("KMsgProcessor::rcv_internal() rcvd SID: ", 
		pan::blob(static_cast<const void*>(sidframe.data()), sidframe.size()));	
#endif
...
KMsgProcessor::init()
{
	...
	pan::log_DEBUG("KMsgProcessor connecting admin: ", _out_addr.c_str());	
	#endif
