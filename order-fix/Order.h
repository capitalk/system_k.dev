...
#include "order_book/KBBO.h" // for SYMBOL_LEN
...
typedef enum exec_trans_type exec_trans_type_t;
...
typedef enum ord_status ord_status_t;
...
typedef enum exec_type exec_type_t;


// Order Types - FIX tag 40 
typedef char ord_type_t;
...
typedef int ord_reject_reason_t;
...
*/
        ...
        void set(capkproto::execution_report& er);

        order_id_t getOid() { return _oid;}
        order_id_t getOrigClOid() { return _origClOid;}
        const char* getExecId() { return _execId;}
        exec_trans_type_t getExecTransType() { return _execTransType;}
        ord_status_t getOrdStatus() { return _ordStatus;}
        exec_type_t getExecType() { return _execType;}
        const char* getSymbol() { return _symbol;}
        const char* getSecType() { return _secType;}
        side_t getSide() { return _side;}
        double getOrdQty() { return _orderQty;}
        ord_type_t getOrdType() { return _ordType;}
        double getPrice() { return _price;}
        double getLastShares() { return _lastShares;}
        double getLastPrice() { return _lastPrice;}
        double getLeavesQty() { return _leavesQty;}
        double getCumQty() { return _cumQty;}
        double getAvgPrice() { return _avgPrice;}
        time_in_force_t getTimeInForce() { return _timeInForce;}
        timespec getTransactTime() { return _transactTime;}
        std::string getTransactTimeStr() { return _transactTimeStr;}
        std::string getExecInstStr() { return _execInstStr;}
        handl_inst_t getHandlInst() { return _handlInst;}
        ord_reject_reason_t getOrdRejectReason() { return _ordRejReason;}
        double getMinQty() { return _minQty;}

	private: 
		order_id_t _oid;
		order_id_t _origClOid;
		char _execId[EXEC_ID_LEN];	
		exec_trans_type_t _execTransType;
		ord_status_t _ordStatus;
		exec_type_t _execType;
		...
		double _cumQty;
		double _avgPrice;
		time_in_force_t _timeInForce;
		timespec _transactTime;
        std::string _transactTimeStr;
        std::string _execInstStr;
		handl_inst_t _handlInst;
		ord_reject_reason_t _ordRejReason;
        double _minQty;
