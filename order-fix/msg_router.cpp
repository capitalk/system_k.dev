...
MsgRouter::MsgRouter(zmq::context_t* context, 
...
// in order to get those two fields
	...
	pan::log_DEBUG("MsgRouter RCV header 1 [", 
	...
	pan::log_DEBUG("MsgRouter RCV header 2 [", 
	...
	pan::log_DEBUG("MsgRouter RCV msg_type [", 
		pan::integer(msg_type.size()), "] thread(", strstr.str().c_str(), ")");

	pan::log_DEBUG("MsgRouter RCV data [", 
...
#ifdef LOG
		pan::log_DEBUG("MsgRouter: inproc addr: ", _inprocAddr.c_str());	
		pan::log_DEBUG("MsgRouter: stop requested: ", pan::boolean(_stopRequested));	
#endif
        
		pan::log_DEBUG("MsgRouter creating socket");	
        _inproc = new zmq::socket_t(*_context, ZMQ_DEALER);
        assert(_inproc);
		pan::log_DEBUG("MsgRouter connecting");	
...
void 
MsgRouter::repMsg(order_id_t& oid) 
{
...
void
MsgRouter::setOrderInterface(capk::OrderInterface& interface) 
{
	this->_interface = &interface;
}

capk::OrderInterface*
MsgRouter::getOrderInterface() 
{
	return _interface;
}
