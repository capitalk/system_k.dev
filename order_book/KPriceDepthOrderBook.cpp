...
void PriceDepthOrderBook::print_iter_range(std::ostream& out, Iter start, 
    Iter end, capitalk::Side side) const 
{ 
    int i = 1; 
...
void PriceDepthOrderBook::print(std::ostream& out) const
{
    ...
    out << "\n"; 
    print_iter_range(out, _bidPrices.begin(), _bidPrices.end(), BID); 
    print_iter_range(out, _offerPrices.begin(), _offerPrices.end(), OFFER);     
    //std::flush(out);
}
