...
std::ostream& 
operator<<(std::ostream& out, const capk::KBook& b)
{
    //out << "OB,";
...
uint32_t
KBook::getTotalVolumeAtLimit(Side_t buySell, double price) 
{
    KTree::iterator it;
    if (buySell == BID) {
...
uint32_t
KBook::getOrderCountAtLimit(Side_t buySell, double price) 
{
    KTree::iterator it;
    if (buySell == BID) {
        it = _findLimit(_bidTree, price);
...
double
KBook::bestPriceVolume(Side_t buySell)
{
...
double
KBook::bestPrice(Side_t buySell)
{
    //printLevels(buySell);
    if (buySell == BID) {
...
void
KBook::printLevels(Side_t buySell)
{
    if (buySell == BID) {
        KTree::reverse_iterator bit = _bidTree.rbegin();
