...
capk::PriceDepthEntry* bid1;
capk::PriceDepthEntry* bid2; 

TEST(BookTests, InitiallyEmpty) { 
    capk::PriceDepthOrderBook book("EUR/USD", 100); 
    EXPECT_EQ(book.numBids(), 0); 
    EXPECT_EQ(book.numOffers(), 0); 	
} 

TEST(BookTests, FindById) { 
    capk::PriceDepthOrderBook book ("EUR/USD", 10);
    book.add(bid1); 
    book.add(bid2); 
    capk::PriceDepthEntry* found = book.findEntryById(bid1->id()); 
...
TEST(BookTests, FindByLevel) { 
    ...
    book.add(bid1); 
    book.add(bid2); 
    capk::PriceDepthEntry* found = book.walkToBidLevel(1); 
...
TEST(BookTests, FindByPrice) { 
    ...
    book.add(bid1); 
    book.add(bid2); 
    capk::PriceDepthEntry* found = book.walkToBidPrice(2.0); 
...
TEST(BookTests, DepthLimit) { 
    capk::PriceDepthOrderBook book("EUR/USD", 1); 
    book.add(bid1); 
    book.add(bid2);  
...
capk::PriceDepthOrderBook  pdob("TESTBOOK", 10);     
